...
def allmovies():
    return pass_data(type="allmovies")
@app.route("/getsinglemovie",methods=["GET"])
def getsinglemovie():
    return pass_data(type="movies", target_id = request.args.get('id'))

@app.route("/getlikessingle",methods=["GET"])
def getsinglelike():
    return pass_data(type="likes", target_id = request.args.get('id'))

@app.route("/getlikes",methods=["GET"])
def getlikes():
    return pass_data(type="alllikes")
...
def pass_data(get_userdata = None,type = None, target_id = None):
    ...
    if type == "register":
        ...
        return obj.show_movies()
    elif type == "movies":
        return obj.show_movies_single(target_id)
    elif type == "likes":
        return obj.show_likes_single(target_id)
    elif type == "alllikes":
        return obj.show_likes()
